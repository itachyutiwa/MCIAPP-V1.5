...
def to_camel_case(column_name):
    ...
    if column_name.lower() in ["date reception", "date_reception"]:
...
def critere_benef(file, ante=pd.to_datetime(str(start_date_entry.get()), dayfirst=True), post=pd.to_datetime(str(end_date_entry.get())), dayfirst=True):
    ...
    sous_dossier_ = "RESULTATS"
    dossier_complet = os.path.join(nom_dossier, sous_dossier)
    dossier_complet_ = os.path.join(nom_dossier, sous_dossier_)
    ...
    # Faites la même vérification pour le sous-dossier_
    if not os.path.exists(dossier_complet_):
        os.makedirs(dossier_complet_, exist_ok=True)
    else:
        os.makedirs(dossier_complet_, exist_ok=True)
    ...
    if len(df_production) != 0:
        path_df_production = os.path.join(
            nom_dossier, sous_dossier_, "PRODUCTION.xlsx")
...
def critere_emission(file, post=pd.to_datetime(str(end_date_entry.get()), dayfirst=True)):
    ...
    police_emission = emission.groupby(["gestionnaire"])["mtPrimeNet"].agg(
...
def critere_conso(file):
    ...
    rs = Prestation().resultats(selected_beneficiaire_path, selected_emission_path,
                                selected_consommation_path, date_fin=end_date_entry.get(), conso_ctx=int(seuil_entry.get()), courtier=COUTIER_entry.get())
    ...
    if len(df_prestation) != 0:
        path_df_prestation = os.path.join(
            nom_dossier, sous_dossier_, "PRESTATION.xlsx")
...
def resume(file_benef, file_emission, file_conso, seuil_ctx, ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    if len(ENFANT_SUP_25_CONSO) != 0:
        ...
        try:
            ENFANT_SUP_25_CONSO = ENFANT_SUP_25_CONSO.drop(columns=['Unnamed: 0'])
    ...
    if len(ADULTES_SUP_60_CONSO) != 0:
        path_ADULTES_SUP_60_CONSO = os.path.join(
            nom_dossier, sous_dossier, "ADULTE_SUP_60_CONSO.xlsx")
        try:
            ADULTES_SUP_60_CONSO = ADULTES_SUP_60_CONSO.drop(columns=['Unnamed: 0'])
