...
class Prestation:
    ...
    def conso_autorisees(self, file_benef, file_cotisation, file_conso, courtier):
        rs = ""
        nom_dossier = str(courtier)
        sous_dossier = "RESULTATS"
        dossier_complet = os.path.join(nom_dossier, sous_dossier)
        #.to_excel(dossier_complet+ '/' + 'ECARTS CONSO.xlsx')
    ...
    def periode_couverture(self, file_benef, file_cotisation, file_conso, date_fin,courtier):
        rs = ""
        nom_dossier = str(courtier)
        sous_dossier = "RESULTATS"
        dossier_complet = os.path.join(nom_dossier, sous_dossier)
    ...
    def double_conso(self, file_benef, file_cotisation, file_conso):
        ...
        consommation = self.database(file_benef, file_cotisation, file_conso)[
            "db"]["consommation"]
        dble_conso = set(consommation[consommation.duplicated(
            keep=False)]["numPolice"].apply(lambda x: str(x)).tolist())
        if len(list(dble_conso)) == 0:
            rs=rs
        elif len(list(dble_conso)) != 0:
            rs = f"{list(set(dble_conso))} : police(s) avec double prestations."
        return {
            "db_conso": rs
        }
    ...
    def depense_imortantes(self, file_benef, file_cotisation, file_conso, conso_ctx):
        ...
        consommation_impt = consommation[consommation["montantPaye"]>= conso_ctx]
    ...
    def resultats(self, file_benef, file_cotisation, file_conso, date_fin, conso_ctx=1000000,courtier=''):
        conso_autorisees = self.conso_autorisees(
            file_benef, file_cotisation, file_conso, courtier)["conso_auto"]
        periode_couverture = self.periode_couverture(
            file_benef, file_cotisation, file_conso, date_fin,courtier)["periode_couv"]
